...
describe('get data test', () => {
  ...
  it('convertCurrency should return throw BadRequestException if from is empty', async () => {
    ...
    expect(async () => await convertCurrency('', 'USD')).rejects.toThrow(BadRequestException);
  });
  ...
  it('getData should return throw BadRequestException if currency is empty', async () => {
    const mockData = {};
    axiosMock.onGet(
      'https://www.alphavantage.co/query' +
      '?function=EARNINGS_CALENDAR' +
    ...
    expect(async () => await getData([], 'USD')).rejects.toThrow(BadRequestException);
  });

  it('getData should return throw BadRequestException if targetCurrency is empty', async () => {
    const mockData = {};
    axiosMock.onGet(
      'https://www.alphavantage.co/query' +
      '?function=EARNINGS_CALENDAR' +
